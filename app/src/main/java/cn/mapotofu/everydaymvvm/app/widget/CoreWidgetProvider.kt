...
import cn.mapotofu.everydaymvvm.app.util.PrefsUtil
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        for (appWidgetId in appWidgetIds) { //自动切换
            if (Actions.NEXT_DAY == intent.action) {
                PrefsUtil.save(Constants.NEXT_DAY_STATUS + appWidgetId, true)
            } else if (Actions.PREVIOUS_DAY == intent.action) {
                PrefsUtil.save(Constants.NEXT_DAY_STATUS + appWidgetId, false)
            }
